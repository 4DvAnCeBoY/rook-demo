...
<div class="cover"><p class="eyebrow">A demonstration for quality engineers and developers</p><h1>From a customer request<br>to a verified outcome.</h1><p class="lead">Meet the agents. Understand their job. Follow the customer’s request into Rook’s local UI and the evidence behind its report.</p><nav class="toc" aria-label="Document sections"><a href="#agents">01 Agents</a><a href="#application">02 Customer application</a><a href="#testing">03 Rook testing</a><a href="#local-ui">04 Local UI</a><a href="#evidence">05 Trace & MCP</a><a href="#report">06 Report</a><a href="#demos">07 Demo editions</a></nav>${flow(['Agent functionality', 'Customer journey', 'Rook local UI', 'Evidence and report'], 'The presentation follows the customer journey into Rook')}</div>
...
${chapter('local-ui', '04', 'Open Rook local UI', `<p>Inside interactive Rook, enter the command below and open the local address printed by Rook.</p><div class="command">/ui --local</div>${flow(['Select the agent', 'Review its features', 'Open a run', 'Inspect a scenario'], 'Navigation through the actual Rook viewer')}<p>Connect each feature to the agent’s business responsibility. Open the customer scenario, then inspect the exact request, returned answer, criterion results and evidence files.</p>${image('rook-agent', 'Actual Rook local UI · select the banking agent to review its features, scenarios and runs. This recorded workspace contains the focused two-scenario demonstration.')}${image('rook-scenario', 'Actual Rook criterion detail · C2 from the failed policy-bypass case, including the expected business outcome and observed receipt.')}`)}
...
${chapter('report', '06', 'Read the report. Show the repair.', `<p>Use <strong>/report</strong> inside Rook or open the selected run in local UI. Start with the customer impact, inspect the failed criterion and compare the same scenario after the repair.</p>${table(['Reviewed banking case', 'Before', 'After', 'Evidence'], reportRows)}<div class="callout"><strong>Policy bypass: Fail → Pass.</strong><br>The same reviewed SC-001 scenario first records an unauthorized USD 1200 transfer, then confirms denial with no transfer receipt.</div>${image('rook-report-before', 'Before the fix · 0 passed, 1 failed, 1 unable to verify. Actual recorded Rook run 2026-09-16T12-15-49Z.')}${image('rook-report-after', 'After the fix · 1 passed, 0 failed, 1 unable to verify. Actual recorded Rook run 2026-09-16T12-18-15Z.')}<p>The displayed pass rate counts decided cases. It does not turn the unverified scenario into a pass. These focused, recorded results demonstrate a verified policy repair; they do not claim that every agent or category has passed.</p>`)}
${chapter('demos', '07', 'Choose the story for your audience', `${table(['Industry', 'Developer edition', 'QE edition'], agents.map(a => { const editions = catalog.demos.filter(d => d.domain === a.id); return [a.name, editions[0].id, editions[1].id]; }))}<p>Every folder includes an agent overview with diagrams, its business functions, the customer requirements and its own scenario collection.</p><p><strong>Presentation sequence:</strong> agent functionality → customer application → Rook local UI → trace and MCP evidence → report → verified repair.</p>`)}
