...
Follow the [interactive demo guide](testing-with-rook.md). Commands such as `/guide`, `/run`, `/report` and `/ui --local` are entered inside Rook. Rook sign-in is separate from your model key; both services may charge for usage.
