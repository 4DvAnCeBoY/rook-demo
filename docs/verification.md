...
Finished videos, original captures, full workspaces, review notes and edit manifests remain local under `artifacts/local/agent-assurance`. The delivery manifest records actual speech-led durations. The [export validation](../media/video-validation.json) records full decoding, audio levels, caption checks and unmuted browser playback for all nine films. These selected checks do not establish that every prepared category passes.
