    ...
    try {
      ...
      return await locked(id, async () => {
        ...
        if (action === 'chat' && req.method === 'POST') {
          ...
          try { return send(200, await executeTurn(domain, session, body.goal, options.modelOptions)); }
          catch (error) { return send(error.evidence ? 502 : 400, { error: error.message, ...(error.evidence ? { evidence: error.evidence } : {}) }); }
          finally { await save(session); }
        }
